              ...
              <ul>
                <li class="dropdown"><a><span>Category</span> <i
                  ...
                  <ul>
                    ...
                    <li class="dropdown"><a href='<c:url value="/sortProducts/${category1.name}"/>'><span>${category1.name}</span></a>

                      <ul>
                        <c:forEach items="${category1.categories}" var="child">
                        <li class="dropdown"><a href='<c:url value="/sortProducts/${child.name}"/>'><span>${child.name}</span></a>
                          <ul>
                            <c:forEach items="${child.categories}" var="subChild">
                              <li><a href='<c:url value="/sortProducts/${subChild.name}"/>'>${subChild.name}</a></li>
                            </c:forEach>
                          </ul>
                          </c:forEach>
                      </ul>
